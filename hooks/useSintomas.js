...
const useSintomas = () => {
    ...
    useEffect(() => {
        const obtenerSintomas = async () => {
            ...
            const url = `https://healthservice.priaid.ch/symptoms?token=${token}&format=json&language=es-es`;
            const api = await fetch(url);
            const sintomas = await api.json()
            guardarSintomas(sintomas);
        }
        obtenerSintomas();
    }, []);
